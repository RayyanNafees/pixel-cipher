...
def matrix(arr: list[object], order: tuple[int, int]) -> list[list[object]]:
    ...
    Returns:
        list[list[object]]: A list of {rows} lists with {columns} obejcts representing the matrix
    '''

    rows, cols = order
    assert rows * cols == len(arr
